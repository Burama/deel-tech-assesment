...
export const getBestProfession = async (
  ...
  const professions = await ProfileModel.findAll({
    attributes: ['profession', [fn('SUM', col('price')), 'totalPaid']],
    include: [
      {
        model: ContractModel,
  ...
  const bestProfession = professions.reduce(
    (bestProfession: any, currentProfession: any) => {
      return currentProfession!.totalPaid > bestProfession.totalPaid
        ? currentProfession
        : bestProfession;
    },
    professions[0],
  ...
  return [...bestClients]
    .sort(
      (firstClient, secondClient) =>
        secondClient.dataValues.totalPaid - firstClient.dataValues.totalPaid,
    )
    .slice(0, limit);
};
