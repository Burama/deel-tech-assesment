...
export const transferFunds = async (senderClient: Profile, receiverClient: Profile, amount: number): Promise<void> => {
  const senderClientWithNewBalance = updateProfileWithNewBalance(senderClient, senderClient.balance - amount);
  const receiverClientWithNewBalance = updateProfileWithNewBalance(receiverClient, receiverClient.balance + amount);
