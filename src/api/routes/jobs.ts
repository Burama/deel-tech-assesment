...
jobsRouter.get(
  ...
  async (req: RequestWithProfile, res: Response) => {
    ...
    try {
      const jobs = await jobsController.getAllUnpaidForActiveContracts(
        profile.id,
      );
      return res.status(RESPONSE_STATUS_CODE.OK).send(jobs);
    } catch (error) {
      console.error(error);
    ...
    try {
      ...
      switch (error.message) {
        case ERROR_INTERNAL.NOT_CLIENT: {
          return res.status(RESPONSE_STATUS_CODE.NOT_PERMITTED).send({
