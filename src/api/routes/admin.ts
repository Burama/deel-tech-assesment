...
adminRouter.get(
  '/best-profession',
  getProfile,
  async (req: RequestWithProfile, res: Response) => {
    const { startDate, endDate } = req.query;
    const isoDateFormatRegex = /^\d{4}-\d{2}-\d{2}$/;

    if (!startDate || !endDate) {
      return res.status(RESPONSE_STATUS_CODE.BAD_REQUEST).send({
        message: RESPONSE_STATUS_MESSAGE.MISSING_REQUIRED_PARAMS,
      });
    }

    if (
      !isoDateFormatRegex.test(startDate as string) ||
      !isoDateFormatRegex.test(endDate as string)
    ) {
      return res.status(RESPONSE_STATUS_CODE.BAD_REQUEST).send({
    ...
    try {
      const bestProffesionName = await adminController.getBestProfession(
        startDate as string,
        endDate as string,
      );
      return res.status(RESPONSE_STATUS_CODE.OK).send({
      ...
      switch (error.message) {
        ...
        default: {
          return res.status(RESPONSE_STATUS_CODE.INTERNALL_ERROR).send({
...
adminRouter.get(
  ...
  async (req: RequestWithProfile, res: Response) => {
    ...
    try {
      ...
      return res.status(RESPONSE_STATUS_CODE.OK).send(bestClients);
    } catch (error) {
      console.error(error);
